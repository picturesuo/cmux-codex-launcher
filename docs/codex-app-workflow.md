...
`docs/queue.md` holds the next small task, `docs/knowledge.md` holds durable
facts, and `.codex/config.toml` sets repo-scoped model and reasoning defaults.
...
3. Let Codex read `AGENTS.md`, `docs/agent-workflow.md`, `docs/queue.md`,
   `docs/knowledge.md`, and the shared context path printed by the launcher.
...
- OpenAI Codex docs: keep personal defaults in `~/.codex/config.toml`, keep
  repo-specific behavior in `.codex/config.toml`, and configure model,
  reasoning, sandbox, approvals, profiles, and tools deliberately.
